...
function ProjCard({ p, delay, inView }: { p: typeof projects[0]; delay: number; inView: boolean }) {
  return (
    ...
    >
      {/* Preview header — video, image, or gradient */}
      <div style={{ height:140, background:p.preview, position:"relative", flexShrink:0, borderBottom:"1px solid rgba(255,255,255,0.04)", overflow:"hidden" }}>
        ...
        {p.media && p.mediaType === "image" && (
          ...
          <img src={p.media} alt={p.name} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.85}}/>
        )}
        {/* overlay so text stays readable */}
        {p.media && <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(15,15,30,0.6),transparent)"}}/>}

        <span style={{ position:"absolute", top:14, left:16, fontSize:10, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:"rgba(255,255,255,0.4)", zIndex:1 }}>{p.cat}</span>
        <div style={{ position:"absolute", top:10, right:14, display:"flex", gap:8, zIndex:1 }}>
          {p.github && <a href={p.github} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{color:"rgba(255,255,255,0.3)",transition:"color 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.color="#7C63FF")} onMouseLeave={e=>(e.currentTarget.style.color="rgba(255,255,255,0.3)")}><Github size={14}/></a>}
          {p.live && <a href={p.live} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{color:"rgba(255,255,255,0.3)",transition:"color 0.15s"}} onMouseEnter={e=>(e.currentTarget.style.color="#7C63FF")} onMouseLeave={e=>(e.currentTarget.style.color="rgba(255,255,255,0.3)")}><ExternalLink size={14}/></a>}
        </div>
        <span style={{ position:"absolute", bottom:10, right:16, fontSize:34, fontWeight:900, fontStyle:"italic", color:"rgba(255,255,255,0.04)", letterSpacing:"-0.04em", zIndex:1 }}>{p.num}</span>
        {p.featured && <div style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(124,99,255,0.5),transparent)",zIndex:1}}/>}
      </div>
      {/* Content */}
